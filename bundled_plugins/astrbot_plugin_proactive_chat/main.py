...
class ProactiveChatPlugin(
    ...
    def __init__(self, context: star.Context, config: AstrBotConfig) -> None:
        ...
        # 将独立 FastAPI 管理端的 API 通过 AstrBot 插件扩展路由暴露给
        # 原生 Plugin Page；页面仍复用原有业务逻辑和配置存储。
        context.register_web_api(
            "/astrbot_plugin_proactive_chat/<path:subpath>",
            self._native_web_api,
            ["GET", "POST", "PUT", "PATCH", "DELETE"],
            "Proactive Chat native Plugin Page API",
        )
    ...
    async def _native_web_api(self, subpath: str):
        """Proxy native Plugin Page requests to the embedded FastAPI app.

        Args:
            subpath: Path below the plugin extension route, without the ``/api/``
                prefix used by the legacy standalone WebUI.

        Returns:
            A Starlette response produced by the embedded WebUI application.
        """
        server = self.web_admin_server
        if server is None or server.app is None:
            return error_response("主动消息管理端不可用", status_code=503)

        endpoint = str(subpath or "").strip("/")
        if not endpoint or endpoint == "ws" or endpoint.startswith("ws/"):
            return error_response("该管理端通道不支持 Plugin Page 转发", status_code=404)
        ...
        if method == "POST" and body:
            try:
                ...
                decoded = None
            if isinstance(decoded, dict) and decoded.get("__astrbot_method") == "DELETE":
        ...
        headers["X-AstrBot-Plugin-Bridge"] = server.bridge_secret
        params = request.query.items()
        target_path = f"/api/{endpoint}"

        try:
            async with httpx.AsyncClient(
                transport=httpx.ASGITransport(app=server.app),
                base_url="http://astrbot-plugin.local",
            ) as client:
            ...
            logger.error(f"[主动消息] 原生 Plugin Page API 转发失败喵: {exc}")
            return error_response("主动消息管理端请求失败", status_code=502)
